...
import { saveInventoryItem, updateInventoryItem } from '@/lib/inventoryService';
  ...
  async function onSubmit(values: InventoryFormValues) {
    ...
    try {
      ...
      const itemToSave: InventoryItem = {
        ...initialData, // Keep original data for edit
        noData: noData,
        ...values,
        // Ensure required fields are never undefined
        areaRuang: values.areaRuang ?? '',
        jenisBarang: values.jenisBarang ?? '',
        merkTipe: values.merkTipe ?? '',
        satuan: values.satuan ?? '',
        kondisi: values.kondisi ?? '',
        statusBarang: values.statusBarang ?? '',
        indukNoBarang: values.indukNoBarang ?? '',
        indukHurufBarang: values.indukHurufBarang ?? '',
        subJenisBarang: values.subJenisBarang ?? '',
        subKodeJenis: values.subKodeJenis ?? '',
        urutSubBarang: values.urutSubBarang ?? '',
        sumberDana: values.sumberDana ?? '',
        urutBarangDana: values.urutBarangDana ?? '',
        subAreaRuang: values.subAreaRuang ?? '',
        supplier: values.supplier ?? '',
        statusPengadaan: values.statusPengadaan ?? '',
        ...
        kodeRekapHapus: values.statusBarang === 'dihapus' ? `${values.indukHurufBarang || ''}${values.subKodeJenis || ''}-HAPUS` : '-',
      };
      
      if (isEditing) {
        await updateInventoryItem(itemToSave);
      } else {
